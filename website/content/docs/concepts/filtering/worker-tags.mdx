...
It is also possible to set `tags` to point to an environment variable or filepath
in the system, through the `env://` and `file://` URLs, respectively:
...
worker {
  name = "web-prod-us-east-1"
  tags = "file://config/worker/tags.hcl"
}
```

Note that within the env var/file, the syntax becomes slightly different to make
it more appropriate for use in these contexts - Only the keys and values
need to be specified, in JSON or HCL format:
